...
def cost_function(parameters, x_in):
    ...
    d2_g_t = elementwise_grad(elementwise_grad(g_trial_solution, 0))(x_in, parameters)
...
def g_trial_solution(x, parameters):
    ...
    """
    return 1 + x + (x ** 2) * neural_network(parameters, x)
...
def g_exact(x):
    ...
    """
    return np.cos(x) + 2 * np.sin(x) - np.cos(x) * np.log((1 / np.cos(x)) + np.tan(x))
...
if __name__ == '__main__':
    ...
    s, t = implicit_euler(system_deriv, num_steps=nx)
    ...
    max_diff_num = np.max(np.abs(s[-1, 0] - g_analytical))
    mse = np.square(s[-1, 0] - g_analytical).mean()
    print("Mse: ", mse)
    ...
    plt.xlabel('t')
    plt.ylabel('y(t)')
    plt.title('Approximated solutions to Example 4')
    plt.show()
